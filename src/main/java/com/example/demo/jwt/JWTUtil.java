...
public class JWTUtil {
    ...
    public JWTUtil(@Value("${spring.jwt.secret}") String secret) {
        ...
        // application.properties에서 secret 받아와 secretKey 생성
        this.secretKey = new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8),
                Jwts.SIG.HS256.key().build().getAlgorithm());
    }

    public String getId(String token) {
        // verifiedWith()으로 jwt의 시그니처가 동일한지 확인
        // jwt의 payload에서 id(유저) 가져오기
        return Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload().get("id",
                String.class);
    }

    public String getName(String token) {
        return Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload().get("name",
                String.class);
    }
